...
import { getAwsSignature, awsUriEscape, awsUriEscapePath, iso8601, canonicalQueryString } from '../src/aws/signature';
...
function authFor(verb: string, key: string, query?: Record<string, string>): OracleResult {
  ...
  if (verb === 'PUT' && query && query.partNumber) {
    ...
    req = s3.listParts({ Bucket: BUCKET, Key: key, UploadId: query.uploadId, MaxParts: query['max-parts'] ? parseInt(query['max-parts'], 10) : undefined, PartNumberMarker: query['part-number-marker'] ? parseInt(query['part-number-marker'], 10) : undefined });
...
describe('getAwsSignature matches the official AWS SDK v2 (oracle)', () => {
  ...
  const cases: Array<{ name: string; verb: string; key: string; query?: Record<string, string> }> = [
    ...
    { name: 'uploadPart with partNumber and uploadId', verb: 'PUT', key: 'bigfile.bin', query: { partNumber: '1', uploadId: 'u123' } },
    { name: 'listParts with max-parts and part-number-marker', verb: 'GET', key: 'bigfile.bin', query: { uploadId: 'u123', 'max-parts': '10', 'part-number-marker': '3' } },
    { name: 'initiate multipart upload (?uploads)', verb: 'POST', key: 'bigfile.bin', query: { uploads: '' } },
    { name: 'DELETE with uploadId', verb: 'DELETE', key: 'bigfile.bin', query: { uploadId: 'u456' } },
  ];
  ...
  it('uses the current time for the default amz date', () => {
    ...
    const sig = getAwsSignature({
      method: 'GET',
      pathname: '/x',
      headers: { host: `${BUCKET}.s3.amazonaws.com`, 'x-amz-date': iso8601(new Date()), 'x-amz-content-sha256': 'UNSIGNED-PAYLOAD' },
      accessKeyId: AK,
      secretAccessKey: SK,
      region: REGION,
    });
  ...
  it('throws without credentials', () => {
    expect(() => getAwsSignature({
      method: 'GET', pathname: '/x', headers: { host: 'h' },
      accessKeyId: '', secretAccessKey: SK, region: REGION,
...
describe('awsSignUrl matches the official AWS SDK v2 (oracle)', () => {
  ...
  const cases: Array<{ name: string; key: string; urlOptions?: Record<string, any> }> = [
    ...
    { name: 'object key with non-ASCII characters', key: '中文 文件.txt', urlOptions: { expires: 60 } },
    { name: 'response-* headers', key: 'exampleobject', urlOptions: { expires: 60, response: { 'content-disposition': 'attachment', 'content-type': 'text/plain' } } },
    ...
    it(c.name, () => {
      ...
      const official = s3.getSignedUrl('getObject', {
        ...
        ...(c.urlOptions && c.urlOptions.response
          ? { ResponseContentDisposition: c.urlOptions.response['content-disposition'], ResponseContentType: c.urlOptions.response['content-type'] }
          : {}),
      });
      const mine = awsSignUrl(
        { accessKeyId: AK, accessKeySecret: SK, region: REGION, bucket: BUCKET, secure: true, stsToken: 'TOKEN' },
        c.key,
        c.urlOptions as any
      );
  ...
  it('defaults to 1800s validity like the OSS entry', () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date('2026-09-01T00:00:00.000Z'));
    const url = awsSignUrl({ accessKeyId: AK, accessKeySecret: SK, region: REGION, bucket: BUCKET, secure: true }, 'exampleobject');
