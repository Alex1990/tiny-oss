...
function mockRequest(url: string, body?: string, headers: Record<string, string> = {}) {
  ...
  return {
    ...
    body,
    headers: {
      set: (name: string, value: any) => { map.set(name, String(value)); },
...
function officialSharedKey(url: string, body: string | undefined, headers: Record<string, string>): string {
...
check('Put Blob matches official for identical headers', () => {
  ...
  const url = 'https://myaccount.blob.core.windows.net/mycontainer/dir/my%20file.txt?comp=block&blockid=MDAwMDE%3D';
...
check('GET without body matches official', () => {
  ...
  const { authorization, date } = officialSharedKey(url, undefined, { 'x-ms-version': '2020-12-06' });
  const mine = getSharedKeyAuthorization({
    verb: 'GET',
...
check('UTF-8 blob name with query matches official', () => {
  const url = 'https://myaccount.blob.core.windows.net/mycontainer/%E7%9B%AE%E5%BD%95/%E6%96%87%E4%BB%B6.txt?comp=metadata&timeout=20';
  const { authorization, date } = officialSharedKey(url, undefined, { 'x-ms-version': '2020-12-06' });
  const mine = getSharedKeyAuthorization({
    verb: 'GET',
    headers: { 'x-ms-date': date, 'x-ms-version': '2020-12-06' },
...
try {
  check('read URL (space in name)', () => {
    const oracle = generateBlobSASQueryParameters(
      { containerName: 'mycontainer', blobName: 'dir/file name.txt', permissions: 'r', expiresOn: new Date(FixedDate.now() + 60000), version: '2020-12-06' },
  ...
  check('write URL with UTF-8 blob name', () => {
    const oracle = generateBlobSASQueryParameters(
      { containerName: 'mycontainer', blobName: '目录/文件.txt', permissions: 'w', expiresOn: new Date(FixedDate.now() + 300000), version: '2020-12-06' },
      new StorageSharedKeyCredential(ACCOUNT, ACCOUNT_KEY)
    );
  ...
  check('response-header overrides', () => {
    const oracle = generateBlobSASQueryParameters(
      {
        containerName: 'mycontainer', blobName: 'a.txt', permissions: 'r', expiresOn: new Date(FixedDate.now() + 60000), version: '2020-12-06',
        cacheControl: 'max-age=3600', contentType: 'text/plain', contentDisposition: 'attachment',
      },
      new StorageSharedKeyCredential(ACCOUNT, ACCOUNT_KEY)
    );
    const url = azureSignUrl(OPTIONS, 'a.txt', {
      expires: 60,
      response: { 'cache-control': 'max-age=3600', 'content-type': 'text/plain', 'content-disposition': 'attachment' },
    });
    assert.equal(new URL(url).searchParams.get('sig'), oracle.signature);
...
check('documentation vector', () => {
  ...
  assert.equal(
    getCanonicalizedAzureHeaders(headers) + getCanonicalizedAzureResource('myaccount', '/mycontainer', { comp: 'metadata', restype: 'container', timeout: '20' }),
