...
function mockTransport() {
  ...
  setTransport(async (url: string, opts: any) => {
    ...
    if (url.indexOf('?uploads') > -1) {
      return { data: '<InitiateMultipartUploadResult><UploadId>u1</UploadId></InitiateMultipartUploadResult>', headers: {}, status: 200, statusText: 'OK' };
    }
    if (opts.method === 'POST' && url.indexOf('?uploadId') > -1) {
      return { data: '<CompleteMultipartUploadResult><ETag>"e"</ETag></CompleteMultipartUploadResult>', headers: {}, status: 200, statusText: 'OK' };
    }
