...
import { put, multipartUpload, signatureUrl, bindOptions } from '../src/aws/index';
...
/**
 * AWS S3 integration tests. They need a real AWS account: set
 * AWS_ACCESS_KEY_ID/AWS_ACCESS_KEY_SECRET/AWS_BUCKET/AWS_REGION in .env.
 * When the config is absent each case is skipped, so CI without
 * credentials stays green.
 */
async function getAwsConfig(): Promise<AwsConfig> {
  const res = await fetch('http://localhost:8080/api/aws-config');
  return res.json() as Promise<AwsConfig>;
}
...
describe('aws integration', () => {
  ...
  it('bindOptions put', async ({ skip }) => {
    if (!configured) skip();
    const content = 's3 bindOptions put';
    const objectName = getObjectName();
