...
describe('canonicalized strings (MSDN documentation vector)', () => {
  it('matches the documented Get Blob example', () => {
    ...
    expect(getCanonicalizedAzureResource('myaccount', '/mycontainer', {
      comp: 'metadata',
      restype: 'container',
      timeout: '20',
  ...
  it('sorts query parameters lexicographically', () => {
    expect(getCanonicalizedAzureResource('a', '/b', { z: '1', a: '2', m: '3' }))
      .toBe('/a/b\na:2\nm:3\nz:1');
  });

  it('drops query parameters without a value (official SDK rule)', () => {
    expect(getCanonicalizedAzureResource('a', '/b', { comp: '', restype: 'container' }))
      .toBe('/a/b\nrestype:container');
  });
...
describe('azure request', () => {
  ...
  it('signs a Put Blob with x-ms-date, x-ms-version and SharedKey', async () => {
    ...
    expect(opts.headers['x-ms-date']).toMatch(/^[A-Z][a-z]{2}, \d{2} [A-Z][a-z]{2} \d{4} \d{2}:\d{2}:\d{2} GMT$/);
  ...
  it('puts sub-resources in the query and skips x-ms-blob-type for block ops', async () => {
    ...
    expect(calls[0].url).toBe('https://myaccount.blob.core.windows.net/mycontainer/blob.txt?comp=block&blockid=MDAwMDE%3D');
...
describe('azure entry point', () => {
  ...
  it('multipartUpload runs Put Block x3 then Put Block List with the right block ids', async () => {
    ...
    const result = await multipartUpload(OPTIONS, 'big.bin', new Uint8Array(300000), { partSize: 100 * 1024, parallel: 2 });
  ...
  it('multipartUpload meta uses the x-ms-meta- prefix on the final Put Block List', async () => {
    ...
    await multipartUpload(OPTIONS, 'k', new Uint8Array(150000), { partSize: 100 * 1024, meta: { title: 't' } });
