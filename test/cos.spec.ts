...
describe('resolveCosHost', () => {
  it('builds bucket.cos.region.myqcloud.com', () => {
    expect(resolveCosHost(COS_OPTIONS)).toBe(
...
describe('cosSignUrl', () => {
  it('emits the q-* parameter signature with encoded key time', () => {
    ...
    expect(
      url.startsWith('https://examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/dir/a.txt?'),
    ).toBe(true)
...
describe('COS request', () => {
  ...
  it('sends empty objectName as the bucket root path', async () => {
    ...
    expect(calls[0].url).toBe(
      'https://examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/?uploads',
    )
  })
...
describe('mini program environment (no Blob global)', () => {
  ...
  it('put accepts an ArrayBuffer and sizes it without touching Blob', async () => {
    ...
    const calls: {
      url: string
      opts: { method: string; headers: Record<string, string>; data?: unknown; total?: number }
    }[] = []
    setTransport(async (url: string, opts) => {
      calls.push({
        url,
        opts: { method: opts.method, headers: opts.headers, data: opts.data, total: opts.total },
      })
      return { data: '<Result/>', headers: {}, status: 200, statusText: 'OK' }
    })
  ...
  it('multipartUpload sizes byte input without touching Blob', async () => {
    vi.stubGlobal('Blob', undefined)
    let initCalls = 0
    const multipartUpload = createMultipartUpload({ metaPrefix: 'x-cos-meta-' } as Protocol, {
...
describe('putSymlink against the COS protocol', () => {
  it('rejects because COS has no symlink API', async () => {
    ...
    await expect(putSymlink(COS_OPTIONS, 'link', 'target')).rejects.toThrow(
