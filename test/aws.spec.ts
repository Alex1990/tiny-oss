...
describe('resolveAwsHost', () => {
  ...
  it('omits the region for us-east-1', () => {
    expect(resolveAwsHost({ ...OPTIONS, region: 'us-east-1' })).toBe(
...
describe('awsSignUrl', () => {
  ...
  it('returns the SigV4 query-parameter scheme', () => {
    ...
    expect(u.searchParams.get('X-Amz-Credential')).toBe(
      'AKIDEXAMPLE/20260901/us-west-2/s3/aws4_request',
    )
  ...
  it('multipartUpload meta uses the x-amz-meta- prefix', async () => {
    ...
    setTransport(async (url: string, opts: any) => {
      ...
      if (url.indexOf('?uploads') > -1) {
        calls.push({ url, opts })
        return {
          data: '<InitiateMultipartUploadResult><Bucket>b</Bucket><Key>k</Key><UploadId>u1</UploadId></InitiateMultipartUploadResult>',
          headers: {},
          status: 200,
          statusText: 'OK',
        }
      ...
      if (url.indexOf('?uploadId') > -1 && opts.method === 'POST') {
        return {
          data: '<CompleteMultipartUploadResult><Location>x</Location><Bucket>b</Bucket><Key>k</Key><ETag>"e"</ETag></CompleteMultipartUploadResult>',
          headers: {},
          status: 200,
          statusText: 'OK',
        }
    ...
    const result = await multipartUpload(OPTIONS, 'k', new Uint8Array([1, 2, 3]), {
      meta: { title: 't' },
    })
