...
- Every operation is a factory over a `Protocol` (`src/protocol.ts`). `src/ops/` holds one file per operation (`createPut`, `createMultipartUpload`, …); each entry (`src/index.ts`, `src/cos|obs|tos|aws|azure/index.ts`) binds a provider's protocol and exports the operations.
...
- `putSymlink` is exported by the OSS and TOS entries; the Azure entry omits `abortMultipartUpload`/`listParts`/`listUploads`/`uploadPartCopy`.
