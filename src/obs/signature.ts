...
const OBS_RESOURCE_PARAMS = [
  'inventory', 'acl', 'backtosource', 'policy', 'torrent', 'logging',
  'location', 'storageinfo', 'quota', 'storageclass', 'storagepolicy', 'mirrorbacktosource',
  'requestpayment', 'versions', 'versioning', 'versionid', 'uploads', 'uploadid',
  'partnumber', 'website', 'notification', 'replication', 'lifecycle', 'deletebucket',
  'delete', 'cors', 'restore', 'tagging', 'append', 'position',
  ...
  'x-image-process', 'x-image-save-object', 'x-image-save-bucket', 'x-oss-process', 'encryption', 'obsworkflowtriggerpolicy',
  'x-workflow-limit', 'x-workflow-prefix', 'x-workflow-start', 'x-workflow-template-name', 'x-workflow-graph-name', 'x-workflow-execution-state',
  'x-workflow-category', 'x-workflow-prefix', 'x-workflow-create', 'directcoldaccess', 'customdomain', 'cdnnotifyconfiguration',
  'metadata', 'dispolicy', 'obscompresspolicy', 'template_name', 'template_name_prefix', 'x-workflow-status',
  'x-workflow-type', 'x-workflow-forbid', 'sfsacl', 'obsbucketalias', 'obsalias', 'rename',
  'name', 'modify', 'attname', 'inventory', 'truncate', 'object-lock',
  'x-obs-security-token', 'publicaccessblock', 'x-obs-trash', 'bucketstatus', 'policystatus', 'x-obs-callback',
];
...
export function canonicalObsQuery(subResource?: Record<string, any>): string {
  ...
  for (const item of items) {
    ...
    if (OBS_RESOURCE_PARAMS.indexOf(key.toLowerCase()) > -1) {
      out.push(listvar.length === 2 && listvar[1] ? `${key}=${decodeURIComponent(listvar[1])}` : key);
    }
  }
...
export function canonicalObsHeaders(headers: Record<string, any>): string {
  ...
  for (const name of names) {
    const value = name.indexOf('x-obs-meta-') === 0
...
export function getObsSignature(opt: ObsSignatureOptions): string {
  ...
  const contentType = headers['Content-Type'] != null
    ...
    : (headers['content-type'] != null ? String(headers['content-type']) : '');
  ...
  const contentMd5 = opt.contentMd5 != null
    ? String(opt.contentMd5)
    : (headers['Content-MD5'] != null
      ? String(headers['Content-MD5'])
      : (headers['Content-Md5'] != null ? String(headers['Content-Md5']) : ''));
